...
def augment_inference_xywh2xyxy(x, augment_imgsz):
    #from augment_imgsz to 640
    ratio = augment_imgsz / 640
    w_new = augment_imgsz
    h_new = 480 * ratio
    y = x.clone() if isinstance(x, torch.Tensor) else np.copy(x)
    y[:, 0] = (x[:, 0] - x[:, 2] / 2) / w_new * 640  # top left x
    y[:, 1] = (x[:, 1] - x[:, 3] / 2) / h_new * 480  # top left y
    y[:, 2] = (x[:, 0] + x[:, 2] / 2) / w_new * 640  # bottom right x
    y[:, 3] = (x[:, 1] + x[:, 3] / 2) / h_new * 480  # bottom right y
...
#包含可见框和全身框
def xywhn2xyxy_vis(x, w=640, h=640, padw=0, padh=0):
    ...
    # 可见框
    y[:, 4] = w * (x[:, 4] - x[:, 6] / 2) + padw  # top left x vis
    y[:, 5] = h * (x[:, 5] - x[:, 7] / 2) + padh  # top left y vis
    y[:, 6] = w * (x[:, 4] + x[:, 6] / 2) + padw  # bottom right x vis
    y[:, 7] = h * (x[:, 5] + x[:, 7] / 2) + padh  # bottom right y vis
    return y
...
def xyxy2xywhn(x, w=640, h=640, clip=False):
    ...
    return y
#包含全身框和可见框
def xyxy2xywhn_vis(x, w=640, h=640, clip=False):
    ...
    y[:, 3] = (x[:, 3] - x[:, 1]) / h  # height
    #可见框
    y[:, 4] = ((x[:, 4] + x[:, 6]) / 2) / w  # x center
    y[:, 5] = ((x[:, 5] + x[:, 7]) / 2) / h  # y center
    y[:, 6] = (x[:, 6] - x[:, 4]) / w  # width
    y[:, 7] = (x[:, 7] - x[:, 5]) / h  # height
    return y
...
def bbox_iou(box1, box2, x1y1x2y2=True, GIoU=False, DIoU=False, CIoU=False, eps=1e-7, grid_cell_flag = False):
    ...
    # Union Area
    if grid_cell_flag:#使用V向量进行计算loss
...
#s_head部分使用的iou计算函数
def s_head_bbox_iou(box1, box2, x1y1x2y2=True, GIoU=False, DIoU=False, CIoU=False, eps=1e-7, grid_cell_flag = False):
    # Returns the IoU of box1 to box2. box1 is 4, box2 is nx4
    box2 = box2.T
...
def non_max_suppression_clusternms(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
                        labels=(), max_box=300):
...
#=--------------------------------------------------------------------重写NMS代码,方便加入DIOU-NMS
def Re_NMS(boxes, scores, iou_thres, GIoU=False, DIoU=False, CIoU=False):
    """
...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    xc = prediction[..., 4] > conf_thres  # candidates#获得满足置信度阈值的bbox的索引
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if multi_label:
            ...
            conf, j = x[:, 5:].max(1, keepdim=True)
            indexes_conf = conf.view(-1) > conf_thres
            x = torch.cat((box, conf, j.float()), 1)[indexes_conf]



        # Filter by class#仅得到想要的类别,比如这里设置相应的类别index,就会滤除其它的类别
        ...
        # i = Re_NMS(boxes, scores, iou_thres)  # 利用重写后的NMS算法
